<div class="creators_list mt-16">
    <?php for ($i = 0; $i < 3; $i++) { ?>
    <div class="creator border_dashboard2 br-10">
        <div>
            <div class="recent_avatar">
                <img src="../../img/png/avatar2.png" alt="">
            </div>
            <div class="creator_info">
                <span>Johnny FD</span>
                <span class="user_country border_dashboard3 br-5 s-20-s"><img src="../../img/png/flags/uk.png"> UK</span>
                <div class="creator_social">
                    <a href="">
                        <img src="../../img/svg/icons/tiktok.svg" alt="">
                    </a>
                    <a href="">
                        <img src="../../img/svg/icons/youtube.svg" alt="">
                    </a>
                </div>
            </div>
        </div>
        <div class="s-20-s">Creating viral reels for Instagram</div>
        <div class="creator_actions">

            <a onclick="offerPopup()" href="#" class="button button_2 button-br_md">View application</a>
        </div>
    </div>
    <?php } ?>
</div>
